...
class BC_ResNetBlock(tf.keras.Model):
    def __init__(self, channels, stride, dilation, p=0.1, transition=False, blocknum=0):
        ...
        if stride != 1:
            ...
            self.pool = tf.keras.layers.AveragePooling2D(pool_size=(2,1))
        else:
            self.dwconv1 = tf.keras.layers.DepthwiseConv2D(
                kernel_size=(3, 1),
                padding="same", # was same
        ...
        self.dropout = tf.keras.layers.SpatialDropout2D(rate=p)
    ...
    def call(self, input_tensor, training=False):
        ...
        #freq-dwconv + ssn (f2)
        if self.transition and self.pool is not None and self.name != "bc_resnet_block_1": # pool only on downsampling transitions
            x = self.dwconv1(x)
            x = self.pool(x) 
        else:
            x = self.dwconv1(x)
        ...
        x = tf.reduce_mean(x, axis=1, keepdims=True) # freq average pool
        ...
        freq = tf.shape(x_res)[1]
        x = tf.repeat(x, repeats=freq, axis=1)
...
model = Sequential([
    ...
    #     batch_size=32
    # ),
    tf.keras.layers.Input(shape=(40, 98, 1), batch_size=100),
    ...
    tf.keras.layers.Reshape((num_classes,))
])
